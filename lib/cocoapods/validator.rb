...
module Pod
  ...
  class Validator
    ...
    #         - The `swift_versions` attribute within in the podspec, in which case the latest version is always chosen.
    #         - The `.swift-version` file present.
    #         - If none are set then the `DEFAULT_SWIFT_VERSION` is used.
    #
    def derived_swift_version
      @derived_swift_version ||= begin
        if !swift_version.nil?
          swift_version
        elsif (version = spec.swift_versions.max) || (version = dot_swift_version)
          version.to_s
        else
    ...
    def warn_for_dot_swift_file_deprecation
      if swift_version.nil? &&  spec.swift_versions.map(&:to_s).empty? && !dot_swift_version.nil?
    ...
    def validate_swift_version
      ...
      unless spec_swift_versions.empty?
        ...
        if !dot_swift_version.nil? && !spec_swift_versions.include?(dot_swift_version)
          message = "Specification `#{spec.name}` specifies an inconsistent `swift_versions` (#{spec_swift_versions.map { |s| "`#{s}`" }.join(', ')}) compared to the one present in your `.swift-version` file (`#{dot_swift_version}`). " \
          ...
          message = "Specification `#{spec.name}` specifies an inconsistent `swift_versions` (#{spec_swift_versions.map { |s| "`#{s}`" }.join(', ')}) compared to the one passed during lint (`#{swift_version}`)."
        end
        unless message.nil?
          error('swift', message)
          return
