  ...
  class Validator
    ...
    def perform_extensive_analysis(spec)
      if spec.test_specification?
        error('spec', "Validating a test spec (`#{spec.name}`) is not supported.") if spec.test_specification?
        return false
      end
    ...
    def perform_extensive_subspec_analysis(spec)
      spec.subspecs.reject { |s| s.test_specification? }.send(fail_fast ? :all? : :each) do |subspec|
