    ...
    class Analyzer
      ...
      def generate_targets
        ...
        aggregate_targets = specs_by_target.keys.map do |target_definition|
          generate_aggregate_target(target_definition, pod_targets)
        end
        if installation_options.integrate_targets?
      ...
      #
      def generate_aggregate_target(target_definition, pod_targets)
        ...
        target.pod_targets = pod_targets.select do |pod_target|
          pod_target.target_definitions.include?(target_definition) &&
              (target_definition.non_inherited_dependencies.map(&:name).include?(pod_target.name) || !pod_target_test_only?(pod_target, pod_targets))
        end
