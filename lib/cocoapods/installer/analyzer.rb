  ...
  class Installer
    ...
    class Analyzer
      ...
      def analyze(allow_fetches = true)
        ...
        @result.specs_by_target = resolver_specs_by_target.each_with_object({}) do |resolver_specs_by_target, hash|
          hash[resolver_specs_by_target[0]] = resolver_specs_by_target[1].map(&:spec)
        end
        @result
      end
      ...
      #
      # @param  [Array<AggregateTarget>] resolver_specs_by_target mapping of targets to resolved specs (containing information about test usage)
      #         aggregate targets
      #
      ...
      def filter_pod_targets_for_target_definition(target_definition, pod_targets, resolver_specs_by_target)
        pod_targets.select do |pod_target|
          included_in_target_definitions = pod_target.target_definitions.include?(target_definition)
          ...
          included_in_target_definitions && !used_by_tests_only
        end
      end
        ...
        if installation_options.deduplicate_targets?
          distinct_targets = resolver_specs_by_target.each_with_object({}) do |dependency, hash|
            ...
            dependent_specs.group_by(&:root).each do |root_spec, resolver_specs|
              ...
              testspecs, specs = all_specs.partition(&:test_specification?)
              ...
              hash[root_spec].keys.find { |k| k == pod_variant }.add_testspecs(testspecs)
            end
          end
          ...
          pod_targets = distinct_targets.flat_map do |_root, target_definitions_by_variant|
            ...
            target_definitions_by_variant.flat_map do |variant, target_definitions|
              generate_pod_target(target_definitions, variant.specs + variant.testspecs, :scope_suffix => suffixes[variant])
            end
          end
