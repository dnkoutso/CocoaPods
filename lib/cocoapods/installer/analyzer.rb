module Pod
  class Installer
    ...
    class Analyzer
      ...
      #
      def initialize(sandbox, podfile, lockfile = nil, plugin_sources = nil, has_dependencies = true, update = false)
        @sandbox  = sandbox
        @podfile  = podfile
        @lockfile = lockfile
      ...
      def analyze(allow_fetches = true)
        ...
        specs_by_source = Hash[resolver_specs_by_target.values.flatten(1).group_by(&:source).map { |source, specs| [source, specs.map(&:spec).uniq] }]
        sources.each { |s| specs_by_source[s] ||= [] }
        AnalysisResult.new(podfile_state, specs_by_target, specs_by_source, specifications, sandbox_state, targets, target_inspections, podfile_dependency_cache)
      end
      ...
      def generate_target(target_definition, target_inspections, pod_targets, resolver_specs_by_target)
        ...
        AggregateTarget.new(sandbox, target_definition.uses_frameworks?, user_build_configurations, archs, target_definition, client_root, user_project, user_target_uuids, pod_targets)
      end
      ...
      # @return [PodTarget]
      #
      def generate_pod_target(target_definitions, target_inspections, pod_specs, scope_suffix: nil)
        if installation_options.integrate_targets?
      ...
      def generate_version_locking_dependencies(podfile_state)
        if update_mode == :all || !lockfile
          ...
          pods_to_update = podfile_state.changed + podfile_state.deleted
      ...
      def generate_sandbox_state(specifications)
        ...
        UI.section 'Comparing resolved specification to the sandbox manifest' do
          sandbox_analyzer = SandboxAnalyzer.new(sandbox, specifications, update_mode?, lockfile)
