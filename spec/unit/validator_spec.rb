...
module Pod
  describe Validator do
    ...
    describe 'swift validation' do
      ...
      def test_swiftpod_with_dot_swift_version(swift_version = '3.1.0')
        validator = test_swiftpod(nil)
        validator.stubs(:dot_swift_version).returns(swift_version)
        validator
      end
      ...
      it 'errors when swift version spec attribute does not match dot swift version' do
        ...
        result.message.should == 'Specification `JSONKit` specifies an inconsistent `swift_versions` (`3.0`, `4.0`) compared to the one present in your `.swift-version` file (`3.2`). ' \
      ...
      it 'errors when swift version spec attribute does not match parameter based swift version' do
        ...
        result = validator.results.first
        result.type.should == :error
        result.message.should == 'Specification `JSONKit` specifies an inconsistent `swift_versions` (`3.0`, `4.0`) compared to the one passed during lint (`3.2`).'
      end
