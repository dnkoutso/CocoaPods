      ...
      describe PodXCConfig do
        describe 'in general' do
          ...
          it 'adds the library build headers and public headers search paths to the xcconfig, with quotes' do
            private_headers = "\"#{@pod_target.build_headers.search_paths(Platform.ios).join('" "')}\""
            public_headers = "\"#{config.sandbox.public_headers.search_paths(Platform.ios).join('" "')}\""
        ...
        describe 'test xcconfig generation' do
          ...
          it 'adds correct header search paths for dependent and test targets' do
            ...
            generator = PodXCConfig.new(@coconut_pod_target, true)
            xcconfig = generator.generate
            xcconfig.to_hash['HEADER_SEARCH_PATHS'].should == '"${PODS_ROOT}/Headers/Private" "${PODS_ROOT}/Headers/Private/CoconutLib" "${PODS_ROOT}/Headers/Public"' \
